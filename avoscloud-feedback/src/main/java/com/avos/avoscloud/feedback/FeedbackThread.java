...
public class FeedbackThread {
                ...
                null, new GenericObjectCallback() {
                  ...
                  public void onSuccess(String content, AVException e) {
                    if (e != null) {
                      ...
                      if (callback != null) {
                        internalOnCommentFetch(callback,commentList, e);
                      }
                      return;
                    } else {
                      try {
                        ...
                        if (callback != null) {
                          internalOnCommentFetch(callback,commentList, new AVException(ex));
                        }
                      }
                    }
                  }
                  ...
                  public void onFailure(Throwable error, String content) {
                    if (callback != null) {
                      internalOnCommentFetch(callback,commentList, new AVException(content, error));
                    }
                  }
                });
          }
  ...
  protected String mapFromObject(Comment comment, boolean threadFlag) {
    ...
    try {
      ...
      if (!threadFlag && !AVUtils.isBlankString(o.getObjectId()) && FeedbackAgent.isPushEnabled()) {
  ...
  private void internalOnCommentFetch(final SyncCallback callback, final List<Comment> comments, final AVException e){
    if(!AVUtils.isMainThread()){
      uiHandler.post(new Runnable() {
        @Override
        public void run() {
          callback.onCommentsFetch(comments,e);
        }
    });
    }else{
      callback.onCommentsFetch(comments,e);
